package de.chilly2go.JasonAltschuler;
/**************************************************************************
...
public class KMeans
{
  ...
  private double     predefinedCentroid;
  /***********************************************************************
   * Data structures
   **********************************************************************/
  ...
  // position vectors of centroids                      dim(2): (k) by (number of channels)
  ...
  /**
   * Empty constructor is private to ensure that clients have to use the Builder inner class to create a KMeans object.
   */
  private KMeans() {}
  ...
  private KMeans(Builder builder)
  {
    ...
    predefinedCentroid = builder.predefinedCentroidValue;
  ...
  public static class Builder
  {
    ...
    private double  predefinedCentroidValue = -1;
    ...
    public Builder predefineCentroidWith(double predefinedCentroidValue)
    {
      this.predefinedCentroidValue = predefinedCentroidValue;
  ...
  public double assignFromCentroid(double value)
  {
    double tempDist;
    double minValue;
    int    minLocation;
    ...
    for (int j = 0; j < k; j++)
    {
      tempDist = distance(new double[]{value}, centroids[j]);
    ...
    return centroids[minLocation][0];
  }
  ...
  private void basicRandSample()
  {
    ...
    for (int i = 0; i < k; i++)
    {
      if (predefinedCentroid != -1)
      {
        centroids[0][0] = predefinedCentroid;
        i++;
      }
  ...
  /**
   * Randomly chooses (without replacement) k data points as initial centroids using a weighted probability distribution
   * (proportional to D(x)^2 where D(x) is the distance from a data point to the nearest, already chosen centroid).
   */
  // TODO: see if some of this code is extraneous (can be deleted)
  private void plusplusWithPredef()
  {
    centroids = new double[k][n];
    ...
    for (int c = 0; c < k; c++)
    {
      ...
      else
      {
        ...
        for (int j = m - 1; j > 0; j--)
        {
          ...
          else // Because of invalid dimension errors, we can't make the forloop go to j2 > -1 when we have (j2-1) in
          // the loop.
          { choose = 0; }
        }
      }
  ...
  /**
   * Signals to stop running KMeans when the marginal improvement in WCSS from the last step is small.
   *
   * @param prevWCSS
   *     error from previous step in the run
   *
